// Carrito de compras - LogiStock
// Modelo: [{ id, codigo, nombre, precio, imagen, cantidad }]
// Precios publicados CON IVA incluido (Chile 19%):
//   totalAPagar = subtotalBruto - descuento
//   neto = totalAPagar / 1.19
//   iva  = totalAPagar - neto
...
function obtenerCarrito() {
  ...
  // Migración: formato viejo sin cantidad o filas duplicadas -> agrupar por id
  const agrupado = {};
  raw.forEach((item) => {
...
function buscarProducto(id) {
  // listaProductos vive en productos.js; fallback por si carrito.js carga primero
...
function agregarAlCarrito(idProducto, cantidadPedida) {
  ...
  // Si estamos en la vista carrito, refrescarla
...
function cargarVistaCarrito() {
  ...
  // Reflejar cupón guardado en el input
  const input = document.getElementById("input-cupon");
...
function actualizarResumen(t) {
  ...
  // Compatibilidad con el span antiguo #total-precio
